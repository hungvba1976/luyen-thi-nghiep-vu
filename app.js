...
const APP_BUILD_ID = "v1.3.0-final-clean-fix11c-fix12b-fix13-fix14";
const DESKTOP_SHELL_UPDATE = !/(Android|iPhone|iPad|iPod|Mobile|webOS|BlackBerry|IEMobile|Opera Mini)/i.test(navigator.userAgent||"");
(function cleanupShellUpdateMarker(){
  try{
    const u=new URL(location.href);
    if(u.searchParams.has("__lt_shell_update")){
      u.searchParams.delete("__lt_shell_update");
...
async function verifyFreshShell(){
  try{
    ...
    if(info?.buildId && info.buildId!==APP_BUILD_ID && !sessionStorage.getItem("shellRefreshPending")){
      ...
      // A unique navigation URL defeats legacy cache-first service workers: the old
      // cache cannot satisfy a request it has never seen, so the browser must go
      // to the network and can receive the new shell.
      const next=new URL(location.href);
      next.searchParams.set("__lt_shell_update",String(info.buildId));
...
async function registerSW(){
  ...
  try{
    ...
    window.addEventListener("online",()=>{reg.update().catch(()=>{});verifyFreshShell();});
    // Desktop tabs are often left open for hours/days. Re-check periodically there
    // only; mobile keeps the already-stable lifecycle behavior unchanged.
    if(DESKTOP_SHELL_UPDATE){
      setInterval(()=>{reg.update().catch(()=>{});verifyFreshShell();},5*60*1000);
    }
