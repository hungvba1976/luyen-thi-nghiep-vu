...
const APP_VERSION = "Mobile V1.3.0 FINAL CLEAN RC · Fix #11+#12+#13";
const APP_BUILD_ID = "v1.3.0-final-clean-fix11-fix12-fix13";
...
function gradeSelectedPracticeAnswers(session){
  ...
  for(const qid of session.questionIds){
    ...
    const q=questionMap(session)[qid];
...
function finishPracticeSession(session=currentSession){
  ...
  practiceFinishInProgress=true;
  currentSession=s;
  try{
    delete s.reviewOnly;
    gradeSelectedPracticeAnswers(s);
    // Persist the fully graded state first, but do not delete it until the
    // result screen has rendered successfully. This makes Finish atomic
    // from the user's point of view and recoverable if rendering/storage fails.
    saveSession(s);
    clearActiveSessionPointer(s.storageKey);
    closeModal();
    renderPracticeResult();
    deleteSession(s.storageKey);
    return true;
  }catch(e){
    ...
    try{
      ...
      markActiveSession(s);
      renderSession();
      showModal(`<div class="eyebrow">CHƯA THỂ KẾT THÚC</div><h3 class="mt8">Ứng dụng chưa hoàn thành được bài luyện.</h3><div class="notice mt12">Phiên luyện đã được giữ lại an toàn. Hãy bấm Kết thúc lại.</div><div class="modal-actions"><button class="btn3d primary" id="retryPracticeFinish">Kết thúc lại</button><button class="btn3d" id="returnPracticeAfterFinishError">Quay lại luyện tập</button></div>`);
      const retry=$("#retryPracticeFinish");
      if(retry)retry.onclick=()=>{practiceFinishInProgress=false;finishPracticeSession(s);};
      const back=$("#returnPracticeAfterFinishError");
      if(back)back.onclick=()=>{closeModal();renderSession();};
    }catch(recoveryError){
      console.error("Không thể phục hồi giao diện sau lỗi kết thúc:",recoveryError);
    }
    return false;
  }finally{
    practiceFinishInProgress=false;
  }
}
function showPracticeFinishConfirm(){
  ...
  $("#reviewBlankBeforeFinish").onclick=()=>{
    s.reviewOnly="blank";
    const i=s.questionIds.findIndex(qid=>practiceStatus(s,qid)==="blank");
    saveSession(s);closeModal();
    if(i>=0)goToQuestionIndex(s,i);else renderSession();
  };
  const finishAnyway=$("#finishAnyway");
  finishAnyway.onclick=guardAction(finishAnyway,async()=>{
    // Keep a direct reference to this Practice session. On some mobile WebViews
    // a modal replacement/back gesture can change globals between click phases.
    finishPracticeSession(s);
  });
...
function renderPracticeResult(){
  ...
  let previous=null,currentSummary=null;
  if(s.practiceResultCommitted && s.practiceResultSummary){
    previous=s.practicePreviousResult||null;
    currentSummary=s.practiceResultSummary;
  }else{
    previous=loadPracticeLastResult(s);
    const completedAt=Date.now();
    currentSummary={kind:"practice",bankId:s.bankId||null,campaignId:s.campaignId||null,bankName:s.bankName,mode:s.mode,newOnly:!!s.newOnly,total:s.questionIds.length,correct:c.correct,wrong:c.wrong,graded,ungraded,percent,completedAt};
    currentSummary.comparison=practiceComparison(previous,currentSummary);
    s.practicePreviousResult=previous;
    s.practiceResultSummary=currentSummary;
    s.practiceResultCommitted=true;
    const persistedSummary={...currentSummary};
    delete persistedSummary.comparison; // Chỉ lưu 1 mốc gần nhất, không xâu chuỗi lịch sử lồng nhau.
    savePracticeLastResult(s,persistedSummary);
    if(s.campaignId)saveCampaignLastResult(s.campaignId,persistedSummary);
    else saveLastResult(persistedSummary);
  }
...
let historyGuardArmed=false,allowHistoryExit=false,systemBackBusy=false;
const HISTORY_GUARD_KEY="luyenthiGuard";
function currentHistoryIsGuard(){ return !!history.state?.[HISTORY_GUARD_KEY]; }
function armHistoryGuard(){
  if(allowHistoryExit)return;
  // Do not trust only an in-memory boolean: Android WebView/PWA can restore a
  // history entry while preserving JS globals differently across lifecycle events.
  if(currentHistoryIsGuard()){historyGuardArmed=true;return;}
  try{
    history.pushState({...(history.state||{}),[HISTORY_GUARD_KEY]:true,buildId:APP_BUILD_ID},"");
    historyGuardArmed=true;
  }catch(e){
    historyGuardArmed=false;
    console.warn("Không tạo được lớp bảo vệ nút Back:",e);
  }
}
...
function handleSystemBack(){
  ...
  // Unknown child view: fail closed to Home instead of ever allowing the phone
  // Back action to escape the app.
  renderHome();return "handled";
}
window.addEventListener("popstate",()=>{
  if(allowHistoryExit)return;
  historyGuardArmed=false;
  if(systemBackBusy){armHistoryGuard();return;}
  systemBackBusy=true;
  try{
    const result=handleSystemBack();
    if(result!=="exit")armHistoryGuard();
  }finally{
    systemBackBusy=false;
  }
});
// Re-arm after mobile lifecycle restores/BFCache. This specifically protects
// hardware/gesture Back; it does not change the app's own Back buttons.
window.addEventListener("pageshow",()=>{if(!allowHistoryExit)armHistoryGuard();});
document.addEventListener("visibilitychange",()=>{if(document.visibilityState==="visible"&&!allowHistoryExit)armHistoryGuard();});
armHistoryGuard();
